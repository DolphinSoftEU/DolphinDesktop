...
class TestUIABackend:
    ...
    def test_type_text_uses_type_keys_with_spaces(self):
        ...
        el.type_keys.assert_called_once_with("hello world", with_spaces=True)
...
class TestWin32Backend:
    ...
    def test_type_text_uses_type_keys(self):
        ...
        el.type_keys.assert_called_once_with("win32 text", with_spaces=True)
...
class TestImageBackend:
    ...
    def test_get_tree_returns_screen_root(self):
        b = ImageBackend()
        tree = b.get_tree(None)
        assert tree["name"] == "screen"
        assert tree["role"] == "image"
...
class TestStubBackends:
    ...
    @pytest.mark.parametrize("cls", STUB_CLASSES)
    def test_stub_find_element_raises_not_implemented(self, cls):
        with pytest.raises(NotImplementedError):
            cls().find_element(None, {})

    @pytest.mark.parametrize("cls", STUB_CLASSES)
    def test_stub_click_raises_not_implemented(self, cls):
        with pytest.raises(NotImplementedError):
            cls().click(None)

    @pytest.mark.parametrize("cls", STUB_CLASSES)
    def test_stub_type_text_raises_not_implemented(self, cls):
        with pytest.raises(NotImplementedError):
            cls().type_text(None, "")

    @pytest.mark.parametrize("cls", STUB_CLASSES)
    def test_stub_get_tree_raises_not_implemented(self, cls):
        with pytest.raises(NotImplementedError):
            cls().get_tree(None)

    @pytest.mark.parametrize("cls", STUB_CLASSES)
    def test_stub_screenshot_raises_not_implemented(self, cls):
        with pytest.raises(NotImplementedError):
            cls().screenshot()
...
class TestRegistry:
    ...
    def test_builtin_list_contains_six_classes(self):
        assert len(_BUILT_IN) == 6
...
class TestDocumentation:
    ...
    def test_architecture_md_has_layer_diagram(self):
        ...
        content = (Path(__file__).parent.parent / "docs" / "architecture.md").read_text(
            encoding="utf-8"
        )
