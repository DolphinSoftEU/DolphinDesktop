...
def _enabled(name: str) -> bool:
    return (env_var(name) or "").strip() == "1"
...
def _find_ws3270() -> str | None:
    ...
    local_appdata = env_var("LOCALAPPDATA") or ""
    candidates = (
    ...
    return next((candidate for candidate in candidates if path_exists(candidate)), None)


def _require_ws3270(message: str) -> None:
    if not is_windows() or _find_ws3270() is None:
...
def _preflight_sap() -> None:
    missing: list[str] = []
    if not (env_var("DOLPHIN_SAP_CONNECTION") or "").strip():
        missing.append("DOLPHIN_SAP_CONNECTION")
    if (
        not (env_var("DOLPHIN_SAP_USER") or "").strip()
        or not (env_var("DOLPHIN_SAP_PASSWORD") or "").strip()
    ):
...
def _preflight_steam() -> None:
    ...
    if not steam_cdp_up() and (env_var("DOLPHIN_STEAM_ALLOW_LAUNCH") or "").strip() != "1":
...
def run_preflight() -> None:
    ...
    if _enabled("DOLPHIN_EXTERNAL_PREFLIGHT"):
        checks.extend((_preflight_pub400, _preflight_sap, _preflight_steam))
