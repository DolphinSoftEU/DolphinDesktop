...
def test_application_repr_and_descendant_detection_are_stable(monkeypatch) -> None:
    from dolphin_desktop import _application

    app = _application.Application.__new__(_application.Application)
    app._backend = "uia"
    app._app = SimpleNamespace(process=123)
    assert repr(app) == "Application(pid=123, backend='uia')"
    monkeypatch.setattr(_application, "_process_parent_map", lambda: {5: [6], 6: [7]})
    assert _application._enumerate_descendant_pids(5) == {6, 7}


def test_application_module_detection_handles_signatures_and_qt_children(monkeypatch) -> None:
    import dolphin_desktop._application as application

    assert application._has_any_signature(["c:/libcef.dll"], ("libcef",)) is True
    assert application._has_any_signature(["user32.dll"], ("libcef",)) is False
    modules = {
        1: ["webview2loader.dll"],
        2: ["libcef.dll", "electron.exe"],
        3: ["libcef.dll"],
        4: ["mshtml.dll"],
    }
    monkeypatch.setattr(application, "_list_modules", lambda pid, **_: modules[pid])
    assert application._webview2_by_modules(1) is True
    assert application._electron_by_modules(2) is True
    assert application._cef_by_modules(3) is True
    assert application._cef_by_modules(2) is False
    assert application._legacy_ie_by_modules(4) is True
    monkeypatch.setattr(application, "_qt_module_info_one", lambda pid: (pid == 8, "6"))
    monkeypatch.setattr(application, "_enumerate_child_pids", lambda pid: [8])
    assert application._qt_module_info(7) == (True, "6")
    assert application._qt_module_info(8) == (True, "6")


def test_application_list_modules_uses_extended_fallback_and_closes_handle(monkeypatch) -> None:
    import dolphin_desktop._application as application

    api = SimpleNamespace(OpenProcess=Mock(return_value=99), CloseHandle=Mock())
    process = SimpleNamespace(
        EnumProcessModulesEx=Mock(side_effect=RuntimeError("unsupported")),
        EnumProcessModules=Mock(return_value=("one", "two")),
        GetModuleFileNameEx=Mock(side_effect=[r"C:\Qt6Core.dll", RuntimeError("gone")]),
    )
    monkeypatch.setitem(sys.modules, "win32api", api)
    monkeypatch.setitem(sys.modules, "win32process", process)

    assert application._list_modules(123, use_extended=True) == [r"c:\qt6core.dll"]
    process.EnumProcessModulesEx.assert_called_once_with(99, application._ENUM_MODULES_ALL)
    api.CloseHandle.assert_called_once_with(99)


def test_application_window_class_probe_ignores_invisible_and_other_process_windows(
    monkeypatch,
) -> None:
    import dolphin_desktop._application as application
    ...
    gui = SimpleNamespace(
        ...
        GetClassName=lambda hwnd: {2: "Chrome_WidgetWin_1", 3: "Other"}[hwnd],
    )
    process = SimpleNamespace(GetWindowThreadProcessId=lambda hwnd: (0, {2: 42, 3: 42}[hwnd]))
    gui.EnumWindows = lambda callback, extra: [callback(hwnd, extra) for hwnd in (1, 2, 3)]
    monkeypatch.setitem(sys.modules, "win32gui", gui)
    monkeypatch.setitem(sys.modules, "win32process", process)
    assert application._electron_by_window_class(42) is True
    assert application._electron_by_window_class(7) is False


def test_application_process_helpers_cover_failure_and_qt_quick_paths(monkeypatch) -> None:
    import dolphin_desktop._application as application

    monkeypatch.setitem(sys.modules, "win32api", None)
    assert application._list_modules(1) == []
    monkeypatch.setattr(ctypes, "WinDLL", Mock(side_effect=OSError("kernel32 unavailable")))
    assert application._process_image_path(1) is None

    monkeypatch.setattr(application, "_list_modules", lambda pid, **kwargs: ["qt6quick.dll"])
    monkeypatch.setattr(application, "_enumerate_child_pids", lambda pid: [])
    assert application._qt_quick_one(1) is True
    assert application._qt_quick_in_modules(1) is True
    monkeypatch.setattr(application, "_list_modules", lambda pid, **kwargs: [])
    monkeypatch.setattr(application, "_enumerate_child_pids", lambda pid: [2])
    monkeypatch.setattr(
        application,
        "_qt_quick_one",
        lambda pid: pid == 2,
...
def test_application_generated_locators_detection_and_lifecycle(monkeypatch) -> None:
    import dolphin_desktop._application as application

    raw = Mock(process=555)
    monkeypatch.setattr(application, "_process_image_path", lambda _: "demo.exe")
    app = application.Application(raw, "uia", owns_process=True, image_path="demo.exe")
    ...
    with pytest.raises(ValueError, match="non-empty"):
        app._generated_criteria("name", "", None, None)
    with pytest.raises(ValueError, match="non-negative"):
        app._generated_criteria("name", "x", None, -1)
    app.set_default_timeout(250)
    assert app.default_timeout_ms == 250
    with pytest.raises(ValueError, match="timeout_ms"):
        app.set_default_timeout(-1)

    monkeypatch.setattr(application, "_webview2_by_modules", lambda pid: pid == 555)
    monkeypatch.setattr(application, "_electron_by_window_class", lambda pid: False)
    monkeypatch.setattr(application, "_electron_by_modules", lambda pid: True)
    monkeypatch.setattr(application, "_cef_by_modules", lambda pid: False)
    monkeypatch.setattr(application, "_legacy_ie_by_modules", lambda pid: True)
    assert app.is_webview2() and app.is_electron() and not app.is_cef() and app.is_legacy_ie()

    app._app.is_process_running.return_value = True
    app.kill()
    raw.kill.assert_called_once_with(soft=False)
    app.detach(session=True)
    assert repr(app) == "Application(pid=555, backend='uia')"
