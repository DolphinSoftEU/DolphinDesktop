...
def test_window_delegates_queries_waits_factories_and_qt_helpers(tmp_path) -> None:
    from dolphin_desktop._exceptions import WaitTimeoutError
    from dolphin_desktop._window import Window

    rect = SimpleNamespace(left=1, top=2, right=11, bottom=22)
    spec = Mock()
    spec.rectangle.return_value = rect
    spec.window_text.return_value = "Main"
    spec.exists.return_value = True
    spec.is_visible.return_value = True
    spec.is_active.return_value = False
    image = Mock()
    spec.capture_as_image.return_value = image
    window = Window(spec)

    assert window.title() == "Main"
    assert window.exists() and window.is_visible() and not window.is_active()
    assert window.bounding_box() == {
        "left": 1,
        "top": 2,
        "right": 11,
        "bottom": 22,
    ...
    assert window.screenshot(tmp_path / "screenshot.png") is image
    ...
    spec.close.assert_called_once_with()
    spec.set_focus.assert_called_once_with()
    window.wait_for_close(0.1)
    window.wait_until_ready(0.1)

    app = Mock()
    window = Window(spec, application=app)
    assert window.qml("root") is app.qml.return_value
    assert window.qt_widget(object_name="edit") is app.qt_widget.return_value
    assert window.graphics_view(object_name="view") is app.graphics_view.return_value
    app.qml.assert_called_once_with("root")
    app.qt_widget.assert_called_once_with(object_name="edit", class_name=None, text=None)
    app.graphics_view.assert_called_once_with(object_name="view")

    with pytest.raises(RuntimeError, match="back-pointer"):
        Window(spec).qml("root")
    spec.wait_not.side_effect = RuntimeError("still visible")
    with pytest.raises(WaitTimeoutError, match="did not close"):
        window.wait_for_close(0)
    spec.wait.side_effect = RuntimeError("busy")
    with pytest.raises(WaitTimeoutError, match="not ready"):
        window.wait_until_ready(0)
