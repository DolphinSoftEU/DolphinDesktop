"""PySide6 reference demo app — exercises every QWidget dolphin_desktop targets.

Run::

    python examples/qt_demo/all_widgets.py

Every interactive widget has a deterministic ``objectName`` (mapped to UIA
``AutomationId``) AND ``accessibleName`` (mapped to UIA Name) so tests can
locate it both ways. The status bar mirrors the last action as
``status: <text>`` for assertions.
...
WINDOW_TITLE = "Dolphin Qt Demo"
WINDOW_OBJECT_NAME = "qt_demo_main_window"


def _named(widget: QWidget, object_name: str, accessible_name: str | None = None) -> QWidget:
    """Set both ``objectName`` and ``accessibleName`` on *widget*.

    UIA exposes ``objectName`` as ``AutomationId`` and ``accessibleName`` as
    ``Name``. Setting both gives tests two stable handles.
    """
...
class CustomDialog(QDialog):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.setObjectName("qt_custom_dialog")
        self.setAccessibleName("Custom Dialog")
        self.resize(320, 160)

        layout = QFormLayout(self)

        self.name_input = _named(QLineEdit(self), "qt_custom_dialog_name", "Name")
        layout.addRow("Name:", self.name_input)

        self.email_input = _named(QLineEdit(self), "qt_custom_dialog_email", "Email")
        ...
        # Tag the OK/Cancel buttons so locators can target them.
        ok_btn = buttons.button(QDialogButtonBox.StandardButton.Ok)
        cancel_btn = buttons.button(QDialogButtonBox.StandardButton.Cancel)
        if ok_btn is not None:
            _named(ok_btn, "qt_custom_dialog_ok", "OK")
        if cancel_btn is not None:
            _named(cancel_btn, "qt_custom_dialog_cancel", "Cancel")
...
class DemoMainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.status.setObjectName("qt_status_bar")
        self.setStatusBar(self.status)
        # Persistent label so the message is always present (showMessage is volatile).
        # Note: accessibleName mirrors the visible text via _set_status, so tests
        # can read the current status by reading this label's UIA Name property.
        self.status_label = QLabel("status: ready", self)
        self.status_label.setObjectName("qt_status_label")
    ...
    def _set_status(self, text: str) -> None:
        ...
        stays in sync with the visible message (tests read it that way).
        """
        msg = f"status: {text}"
    ...
    def _build_menu(self) -> None:
        ...
        menubar.setObjectName("qt_menubar")

        file_menu = menubar.addMenu("&File")
        file_menu.setObjectName("qt_menu_file")
        new_act = QAction("New", self)
        new_act.setObjectName("qt_action_new")
        new_act.triggered.connect(lambda: self._set_status("menu File>New"))
        open_act = QAction("Open...", self)
        open_act.setObjectName("qt_action_open")
        open_act.triggered.connect(lambda: self._set_status("menu File>Open"))
        quit_act = QAction("Quit", self)
        quit_act.setObjectName("qt_action_quit")
        ...
        edit_menu.setObjectName("qt_menu_edit")
        copy_act = QAction("Copy", self)
        copy_act.setObjectName("qt_action_copy")
        copy_act.triggered.connect(lambda: self._set_status("menu Edit>Copy"))
        paste_act = QAction("Paste", self)
        paste_act.setObjectName("qt_action_paste")
        paste_act.triggered.connect(lambda: self._set_status("menu Edit>Paste"))
        edit_menu.addAction(copy_act)
        edit_menu.addAction(paste_act)

        # Nested submenu under View > Zoom > 100% / 200%
        view_menu = menubar.addMenu("&View")
        view_menu.setObjectName("qt_menu_view")
        zoom_menu = view_menu.addMenu("Zoom")
        zoom_menu.setObjectName("qt_menu_zoom")
        for level in ("50%", "100%", "200%"):
            act = QAction(level, self)
            act.setObjectName(f"qt_action_zoom_{level.rstrip('%')}")
    ...
    def _build_toolbar(self) -> None:
        ...
        toolbar.setObjectName("qt_toolbar_main")
        self.addToolBar(toolbar)

        bold_act = QAction("Bold", self)
        bold_act.setObjectName("qt_action_bold")
        ...
        italic_act.setObjectName("qt_action_italic")
    ...
    def _build_central(self) -> None:
        ...
        tabs.setObjectName("qt_main_tabs")
    ...
    def _build_buttons_tab(self) -> QWidget:
        page = QWidget(self)
        page.setObjectName("qt_tab_buttons")
        layout = QVBoxLayout(page)

        ok_btn = _named(QPushButton("OK", page), "qt_btn_ok", "OK")
        ok_btn.clicked.connect(lambda: self._set_status("clicked OK"))
        layout.addWidget(ok_btn)

        cancel_btn = _named(QPushButton("Cancel", page), "qt_btn_cancel", "Cancel")
        cancel_btn.clicked.connect(lambda: self._set_status("clicked Cancel"))
        layout.addWidget(cancel_btn)

        toggle_btn = _named(QPushButton("Toggle", page), "qt_btn_toggle", "Toggle")
        toggle_btn.setCheckable(True)
        toggle_btn.toggled.connect(lambda v: self._set_status(f"toggle={v}"))
        layout.addWidget(toggle_btn)

        disabled_btn = _named(QPushButton("Disabled", page), "qt_btn_disabled", "Disabled")
        disabled_btn.setEnabled(False)
        layout.addWidget(disabled_btn)

        # Checkboxes
        chk_remember = _named(QCheckBox("Remember me", page), "qt_chk_remember", "Remember me")
        chk_remember.toggled.connect(lambda v: self._set_status(f"remember={v}"))
        layout.addWidget(chk_remember)

        chk_updates = _named(QCheckBox("Send updates", page), "qt_chk_updates", "Send updates")
    ...
    def _build_inputs_tab(self) -> QWidget:
        page = QWidget(self)
        page.setObjectName("qt_tab_inputs")
        layout = QFormLayout(page)

        username = _named(QLineEdit(page), "qt_input_username", "Username")
        username.textEdited.connect(lambda t: self._set_status(f"username={t}"))
        layout.addRow("Username:", username)

        password = _named(QLineEdit(page), "qt_input_password", "Password")
        password.setEchoMode(QLineEdit.EchoMode.Password)
        layout.addRow("Password:", password)

        readonly = _named(QLineEdit("read-only", page), "qt_input_readonly", "Readonly")
        readonly.setReadOnly(True)
        layout.addRow("Readonly:", readonly)

        spinner = _named(QSpinBox(page), "qt_spin_count", "Count")
        spinner.setRange(0, 100)
        spinner.setValue(10)
        spinner.valueChanged.connect(lambda v: self._set_status(f"count={v}"))
        layout.addRow("Count:", spinner)

        double_spinner = _named(QDoubleSpinBox(page), "qt_spin_price", "Price")
        ...
        slider = _named(QSlider(Qt.Orientation.Horizontal, page), "qt_slider_volume", "Volume")
        slider.setRange(0, 100)
        slider.setValue(50)
        slider.valueChanged.connect(lambda v: self._set_status(f"volume={v}"))
        layout.addRow("Volume:", slider)

        date = _named(QDateEdit(page), "qt_date_birth", "Birth date")
        date.setDate(QDate(2000, 1, 1))
        date.setCalendarPopup(True)
        layout.addRow("Birth date:", date)

        notes = _named(QPlainTextEdit(page), "qt_input_notes", "Notes")
    ...
    def _build_choices_tab(self) -> QWidget:
        page = QWidget(self)
        page.setObjectName("qt_tab_choices")
        layout = QFormLayout(page)

        combo = _named(QComboBox(page), "qt_combo_language", "Language")
        ...
        editable_combo = _named(QComboBox(page), "qt_combo_city", "City")
        ...
        listbox = _named(QListWidget(page), "qt_list_fruits", "Fruits")
    ...
    def _build_containers_tab(self) -> QWidget:
        page = QWidget(self)
        page.setObjectName("qt_tab_containers")
        layout = QVBoxLayout(page)

        # Tree
        tree = _named(QTreeWidget(page), "qt_tree_files", "Files")
        ...
        # Table with a model
        table = _named(QTableView(page), "qt_table_people", "People")
    ...
    def _build_dialogs_tab(self) -> QWidget:
        page = QWidget(self)
        page.setObjectName("qt_tab_dialogs")
        layout = QVBoxLayout(page)

        info_btn = _named(QPushButton("Show info", page), "qt_btn_show_info", "Show info")
        info_btn.clicked.connect(self._show_info)
        layout.addWidget(info_btn)

        confirm_btn = _named(
            QPushButton("Show confirm", page), "qt_btn_show_confirm", "Show confirm"
        )
        confirm_btn.clicked.connect(self._show_confirm)
        layout.addWidget(confirm_btn)

        input_btn = _named(QPushButton("Show input", page), "qt_btn_show_input", "Show input")
        input_btn.clicked.connect(self._show_input)
        layout.addWidget(input_btn)

        custom_btn = _named(QPushButton("Show custom", page), "qt_btn_show_custom", "Show custom")
    ...
    def _build_dock(self) -> None:
        ...
        dock.setObjectName("qt_dock_properties")
        dock_content = QWidget(dock)
        dock_layout = QVBoxLayout(dock_content)
        info = _named(QLabel("Property pane", dock_content), "qt_dock_label", "Property pane")
    ...
    def _show_confirm(self) -> None:
        ...
        _named(yes_btn, "qt_msg_confirm_yes", "Yes")
        _named(no_btn, "qt_msg_confirm_no", "No")
        box.exec()
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.no_show:
        # Pump events once so widgets are realized, then exit cleanly.
        app.processEvents()
        return 0
